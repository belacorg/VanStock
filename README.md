...
Type the GC number off the label — or scan the label — and get one of four answers: it's on the van and here's the box; it's lent out and here's who has it; there's none left; or it's not on your list at all.
...
- **Scanning** — reads the GC number and description off a British Gas dispatch label, on the phone. Fill the screen with the top of the label. Nothing else on the label is read or kept, and the photo isn't stored.
...
The reasoning behind the shape is in [`CONTEXT.md`](CONTEXT.md) and [`docs/adr/`](docs/adr/). Start with ADR-0002 (what "on board" means), ADR-0009 (why the scanner reads the print) and ADR-0010 (what it keeps).
